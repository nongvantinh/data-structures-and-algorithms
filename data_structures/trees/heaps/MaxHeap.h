/*
Class: MaxHeap.
Purpose: Implement max heap data structure, this class fully support max priority queue.
        ...
        - Third parameter Compare define how max heap mantains its property,
        - it must be std::greater_equal<Elem> or std::greater<Elem>, other comparer is undefined behavious.
*/

#ifndef DSAA_MAX_HEAP_H
#define DSAA_MAX_HEAP_H
...
namespace dsaa
{
  template <typename Elem, typename Alloc = std::allocator<Elem>, typename Compare = std::greater_equal<Elem>>
  class MaxHeap
  {
  public:
    class ConstIterator;
    ...
    CONSTEXPR MaxHeap(const allocator_type &p_allocator = allocator_type()) noexcept;
    // Creates a container's p_size elements with default value.
    CONSTEXPR explicit MaxHeap(const size_type &p_size, const allocator_type &p_allocator = allocator_type());
    // Creates a container's p_size elements and init its element by p_value.
    CONSTEXPR MaxHeap(const size_type &p_size, const_reference p_value, const allocator_type &p_allocator = allocator_type());
    // Creates a container's size equal to p_elements's size and init its elements using p_elements's element.
    CONSTEXPR MaxHeap(const std::initializer_list<value_type> &p_elements, const allocator_type &p_allocator = allocator_type());
    // Creates a container and init its elements using content of IIterator in range (first, last].
    template <typename IIterator>
    CONSTEXPR MaxHeap(const IIterator &p_first, const IIterator &p_last, const allocator_type &p_allocator = allocator_type());
    // Creates a container and copy all emements from p_other.
    CONSTEXPR MaxHeap(const MaxHeap &p_other);
    // Creates a container and copy all emements from p_other.
    CONSTEXPR MaxHeap(const MaxHeap &p_other, const allocator_type &p_allocator);
    // Moves all elements from p_other into this.
    CONSTEXPR MaxHeap(MaxHeap &&p_other) noexcept;
    // Moves all elements from p_other into this.
    CONSTEXPR MaxHeap(MaxHeap &&p_other, const allocator_type &p_allocator) noexcept;
    // Destroys old elements and copy all emements from p_other into this.
    CONSTEXPR MaxHeap &operator=(const MaxHeap &p_other);
    // Destroys old elements and moves all emements from p_other into this.
    CONSTEXPR MaxHeap &operator=(MaxHeap &&p_other) noexcept(std::allocator_traits<allocator_type>::propagate_on_container_move_assignment::value || std::allocator_traits<allocator_type>::is_always_equal::value);
    // Destroys old elements and copy all elements from p_elements.
    CONSTEXPR MaxHeap &operator=(const std::initializer_list<value_type> &p_elements);
    ...
    NODISCARD CONSTEXPR INLINE bool max_heap_property(size_type p_index) const NOEXCEPT;
    // Key function to maintaining heap properties define by Compare.
    CONSTEXPR void recursive_max_heapify(size_type p_parent_index) NOEXCEPT;
    CONSTEXPR void iterative_max_heapify(size_type p_parent_index) NOEXCEPT;
    // Build a heap from an input array.
    CONSTEXPR void build_max_heap() NOEXCEPT;
    NODISCARD CONSTEXPR value_type extract_max() NOEXCEPT;
    NODISCARD CONSTEXPR INLINE value_type get_max() NOEXCEPT;
    ...
    CONSTEXPR iterator increase_key(int64_t p_index, const_reference p_value) NOEXCEPT;
    CONSTEXPR iterator increase_key(int64_t p_index, value_type &&p_value) NOEXCEPT;
    template <class... Args>
    CONSTEXPR iterator increase_key(int64_t p_index, Args &&...p_args) NOEXCEPT;
...
class dsaa::MaxHeap<Elem, Alloc, Compare>::ConstIterator
{
public:
...
template <typename Elem, typename Alloc, typename Compare>
class dsaa::MaxHeap<Elem, Alloc, Compare>::Iterator : public dsaa::MaxHeap<Elem, Alloc, Compare>::ConstIterator
{
public:
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::iterator dsaa::MaxHeap<Elem, Alloc, Compare>::begin() noexcept
{
  return heap_size() ? iterator(m_data.begin().content()) : iterator(nullptr);
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::const_iterator dsaa::MaxHeap<Elem, Alloc, Compare>::begin() const noexcept
{
  return heap_size() ? const_iterator(m_data.begin().content()) : const_iterator(nullptr);
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::const_iterator dsaa::MaxHeap<Elem, Alloc, Compare>::cbegin() const noexcept
{
  return heap_size() ? const_iterator(m_data.begin().content()) : const_iterator(nullptr);
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::iterator dsaa::MaxHeap<Elem, Alloc, Compare>::end() noexcept
{
  return heap_size() ? iterator(m_data.end().content()) : iterator(nullptr);
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::const_iterator dsaa::MaxHeap<Elem, Alloc, Compare>::end() const noexcept
{
  return heap_size() ? const_iterator(m_data.end().content()) : const_iterator(nullptr);
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::const_iterator dsaa::MaxHeap<Elem, Alloc, Compare>::cend() const noexcept
{
  return heap_size() ? const_iterator(m_data.end().content()) : const_iterator(nullptr);
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::reverse_iterator dsaa::MaxHeap<Elem, Alloc, Compare>::rbegin() noexcept
{
  return reverse_iterator(end());
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::const_reverse_iterator dsaa::MaxHeap<Elem, Alloc, Compare>::rbegin() const noexcept
{
  return const_reverse_iterator(end());
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::const_reverse_iterator dsaa::MaxHeap<Elem, Alloc, Compare>::crbegin() const noexcept
{
  return const_reverse_iterator(end());
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::reverse_iterator dsaa::MaxHeap<Elem, Alloc, Compare>::rend() noexcept
{
  return reverse_iterator(begin());
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::const_reverse_iterator dsaa::MaxHeap<Elem, Alloc, Compare>::rend() const noexcept
{
  return const_reverse_iterator(begin());
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::const_reverse_iterator dsaa::MaxHeap<Elem, Alloc, Compare>::crend() const noexcept
{
  return const_reverse_iterator(begin());
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::MaxHeap<Elem, Alloc, Compare>::MaxHeap(const allocator_type &p_allocator) noexcept
    : m_data(p_allocator), m_comparer() {}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::MaxHeap<Elem, Alloc, Compare>::MaxHeap(const size_type &p_size, const allocator_type &p_allocator)
    : m_data(p_size, p_allocator), m_comparer()
{
#ifdef PARAM_CHECK
  if (std::is_same<std::greater<value_type>, Compare>::value)
    throw std::runtime_error("Priority queue can only accept element has difference priority, because Compare is std::greater type.\n");
#endif
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::MaxHeap<Elem, Alloc, Compare>::MaxHeap(const size_type &p_size, const_reference p_value, const allocator_type &p_allocator)
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::MaxHeap<Elem, Alloc, Compare>::MaxHeap(const std::initializer_list<value_type> &p_elements, const allocator_type &p_allocator)
    : m_data(p_elements, p_allocator), m_comparer() { build_max_heap(); }

template <typename Elem, typename Alloc, typename Compare>
template <typename IIterator>
CONSTEXPR dsaa::MaxHeap<Elem, Alloc, Compare>::MaxHeap(const IIterator &p_first, const IIterator &p_last, const allocator_type &p_allocator)
    : m_data(p_first, p_last, p_allocator), m_comparer() { build_max_heap(); }

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::MaxHeap<Elem, Alloc, Compare>::MaxHeap(const MaxHeap &p_other)
    : m_data(p_other.m_data), m_comparer() {}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::MaxHeap<Elem, Alloc, Compare>::MaxHeap(const MaxHeap &p_other, const allocator_type &p_allocator)
    : m_data(p_other.m_data, p_allocator), m_comparer() {}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::MaxHeap<Elem, Alloc, Compare>::MaxHeap(MaxHeap &&p_other) noexcept
    : m_data(std::move(p_other.m_data)), m_comparer() {}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::MaxHeap<Elem, Alloc, Compare>::MaxHeap(MaxHeap &&p_other, const allocator_type &p_allocator) noexcept
    : m_data(std::move(p_other.m_data), p_allocator), m_comparer() {}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::MaxHeap<Elem, Alloc, Compare> &dsaa::MaxHeap<Elem, Alloc, Compare>::operator=(const MaxHeap &p_other)
{
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::MaxHeap<Elem, Alloc, Compare> &dsaa::MaxHeap<Elem, Alloc, Compare>::operator=(MaxHeap &&p_other) noexcept(std::allocator_traits<allocator_type>::propagate_on_container_move_assignment::value || std::allocator_traits<allocator_type>::is_always_equal::value)
{
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::MaxHeap<Elem, Alloc, Compare> &dsaa::MaxHeap<Elem, Alloc, Compare>::operator=(const std::initializer_list<value_type> &p_elements)
{
  m_data = p_elements;
  build_max_heap();
...
template <typename Elem, typename Alloc, typename Compare>
dsaa::MaxHeap<Elem, Alloc, Compare>::~MaxHeap()
{
}
...
template <typename Elem, typename Alloc, typename Compare>
template <typename IIterator>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::assign(const IIterator &p_first, const IIterator &p_last)
{
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::assign(const size_type &p_size, const value_type &p_value)
{
#ifdef PARAM_CHECK
  if (std::is_same<std::greater<value_type>, Compare>::value)
    throw std::runtime_error("Priority queue can only accept element has difference priority, because Compare is std::greater type.\n");
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::assign(const std::initializer_list<value_type> &p_elements)
{
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::reference dsaa::MaxHeap<Elem, Alloc, Compare>::operator[](const size_type &p_index) NOEXCEPT
{
#ifdef PARAM_CHECK
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::const_reference dsaa::MaxHeap<Elem, Alloc, Compare>::operator[](const size_type &p_index) const NOEXCEPT
{
#ifdef PARAM_CHECK
  if (heap_size() <= p_index)
    throw std::out_of_range("p_index out of range exception.\n;");
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::reference dsaa::MaxHeap<Elem, Alloc, Compare>::at(const size_type &p_index)
{
  if (heap_size() <= p_index)
    throw std::out_of_range("p_index out of range exception.\n;");
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::const_reference dsaa::MaxHeap<Elem, Alloc, Compare>::at(const size_type &p_index) const
{
  if (heap_size() <= p_index)
    throw std::out_of_range("p_index out of range exception.\n;");
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::DynamicArray<Elem> &dsaa::MaxHeap<Elem, Alloc, Compare>::data() noexcept
{
  return m_data;
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR dsaa::DynamicArray<Elem> const &dsaa::MaxHeap<Elem, Alloc, Compare>::data() const noexcept
{
  return m_data;
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR bool dsaa::MaxHeap<Elem, Alloc, Compare>::empty() const noexcept
{
  return m_data.empty();
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::size_type dsaa::MaxHeap<Elem, Alloc, Compare>::size() const noexcept
{
  return m_data.capacity();
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::size_type dsaa::MaxHeap<Elem, Alloc, Compare>::heap_size() const noexcept
{
  return m_data.size();
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR bool dsaa::MaxHeap<Elem, Alloc, Compare>::is_leaf(size_type p_index) const noexcept
{
  return heap_size() / 2 <= p_index;
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::size_type dsaa::MaxHeap<Elem, Alloc, Compare>::get_index(const const_iterator &p_position) NOEXCEPT
{
  return m_data.get_index(typename dsaa::DynamicArray<Elem, Alloc>::const_iterator(p_position.content()));
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::iterator dsaa::MaxHeap<Elem, Alloc, Compare>::get_iterator(const size_type &p_index) NOEXCEPT
{
  return iterator(m_data.get_iterator(p_index).content());
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::shrink_size(const size_type &p_size)
{
#ifdef PARAM_CHECK
  if (heap_size() <= p_size)
    throw std::runtime_error("MaxHeap only allow shrink size to smaller size.\n");
#endif
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::size_type dsaa::MaxHeap<Elem, Alloc, Compare>::parent(size_type p_index) const NOEXCEPT
{
#ifdef PARAM_CHECK
  if (0 == p_index)
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::size_type dsaa::MaxHeap<Elem, Alloc, Compare>::left_child(size_type p_index) const NOEXCEPT
{
#ifdef PARAM_CHECK
  if (is_leaf(p_index))
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::size_type dsaa::MaxHeap<Elem, Alloc, Compare>::right_child(size_type p_index) const NOEXCEPT
{
#ifdef PARAM_CHECK
  if (is_leaf(p_index))
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR bool dsaa::MaxHeap<Elem, Alloc, Compare>::max_heap_property(size_type p_index) const NOEXCEPT
{
#ifdef PARAM_CHECK
  if (heap_size() <= p_index)
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::recursive_max_heapify(size_type p_parent_index) NOEXCEPT
{
  ...
  if (key_index != p_parent_index)
  {
    ...
    recursive_max_heapify(key_index);
  }
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::iterative_max_heapify(size_type p_parent_index) NOEXCEPT
{
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::build_max_heap() NOEXCEPT
{
  for (int64_t index((size() - 1) / 2); 0 <= index; --index)
  {
    iterative_max_heapify(index);
  }
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::value_type dsaa::MaxHeap<Elem, Alloc, Compare>::extract_max() NOEXCEPT
{
#ifdef PARAM_CHECK
  if (0 == heap_size())
  ...
  m_data.erase_last();
  iterative_max_heapify(0);
  return result;
}

template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::value_type dsaa::MaxHeap<Elem, Alloc, Compare>::get_max() NOEXCEPT
{
#ifdef PARAM_CHECK
  if (0 == heap_size())
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::iterator dsaa::MaxHeap<Elem, Alloc, Compare>::insert(const_reference p_value) NOEXCEPT
{
  m_data.insert_last(p_value);
  int64_t index(heap_size() - 1);
  while (0 < index && !max_heap_property(index))
  {
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::iterator dsaa::MaxHeap<Elem, Alloc, Compare>::insert(value_type &&p_value) NOEXCEPT
{
  m_data.insert_last(std::move(p_value));
  int64_t index(heap_size() - 1);
  while (0 < index && !m_comparer(m_data[parent(index)], m_data[index]))
  {
    std::swap(m_data[index], m_data[parent(index)]);
    index = parent(index);
  }
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::insert(const size_type &p_size, const_reference p_value) NOEXCEPT
{
#ifdef PARAM_CHECK
  if (std::is_same<std::greater<value_type>, Compare>::value)
    throw std::runtime_error("Priority queue can only accept element has difference priority, because Compare is std::greater type.\n");
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::insert(const std::initializer_list<value_type> &p_elements) NOEXCEPT
{
...
template <typename Elem, typename Alloc, typename Compare>
template <class IIterator>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::insert(const IIterator &p_first, const IIterator &p_last) NOEXCEPT
{
...
template <typename Elem, typename Alloc, typename Compare>
template <class... Args>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::iterator dsaa::MaxHeap<Elem, Alloc, Compare>::emplace(Args &&...p_args) NOEXCEPT
{
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::iterator dsaa::MaxHeap<Elem, Alloc, Compare>::increase_key(int64_t p_index, const_reference p_value) NOEXCEPT
{
#ifdef PARAM_CHECK
  ...
  while (0 < p_index && !max_heap_property(p_index))
  {
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::iterator dsaa::MaxHeap<Elem, Alloc, Compare>::increase_key(int64_t p_index, value_type &&p_value) NOEXCEPT
{
#ifdef PARAM_CHECK
  if (!m_comparer(p_value, m_data[p_index]))
    throw std::invalid_argument("new key is greater than current key.\n");
...
template <typename Elem, typename Alloc, typename Compare>
template <class... Args>
CONSTEXPR typename dsaa::MaxHeap<Elem, Alloc, Compare>::iterator dsaa::MaxHeap<Elem, Alloc, Compare>::increase_key(int64_t p_index, Args &&...p_args) NOEXCEPT
{
#ifdef PARAM_CHECK
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::erase_at(const const_iterator &p_position) NOEXCEPT
{
#ifdef PARAM_CHECK
  if (p_position < begin())
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::erase(const const_iterator &p_first, const const_iterator &p_last) NOEXCEPT
{
#ifdef PARAM_CHECK
  if (p_last < p_first)
...
template <typename Elem, typename Alloc, typename Compare>
CONSTEXPR void dsaa::MaxHeap<Elem, Alloc, Compare>::clear() noexcept
{
  m_data.clear();
}

#endif // !DSAA_MAX_HEAP_H